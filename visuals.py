...
def plot_mutual_information(df_summary: pd.DataFrame, x_field='dil', y_fields=('I_norm',), groupby=['strand', 'frame'],
                            title_prefix='', fig_size=FIG_SIZE, figs_path=FIGS_PATH):
    """
    ...
    for label, df in df_summary.groupby(by=groupby):
        for y_field in y_fields:
            plt.plot(df[x_field], df[y_field], label=label)

    plt.ylim(bottom=0)
    plt.xlim(left=1)
    plt.xticks(range(0, int(max(df_summary.dil)), 3))
...
def plot_mutual_information2(df_kmer: pd.DataFrame, x_field='dil', y_fields=('I',),
                             title_prefix='', fig_size=FIG_SIZE, figs_path=FIGS_PATH):
    """
    Plot mutual information function vs dilation.
    Input df is generated by mutual_information_by_dilation.
