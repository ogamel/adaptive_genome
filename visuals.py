...
def plot_mutual_information(df: pd.DataFrame, x_field='dil', y_fields=('I_norm',),
                            title_prefix='', fig_size=FIG_SIZE, figs_path=FIGS_PATH):
    """
    ...
    plt.ylim(bottom=0)
    plt.xlim(left=1)
    plt.xticks(range(0, int(max(df.dil)), 3))
...
def plot_mutual_information2(df2: pd.DataFrame, x_field='dil', y_fields=('I',),
                            title_prefix='', fig_size=FIG_SIZE, figs_path=FIGS_PATH):
    """
    Plot mutual information function vs dilation.
    Input df is generated by mutual_information_by_dilation.
    ...
    plt.figure(figsize=fig_size)
    for y_field in y_fields:
        for kmer in df2.kmer.unique():
            dfk = df2[df2.kmer == kmer]
            plt.plot(dfk[x_field], dfk[y_field], label=kmer)

    plt.legend()
    # plt.ylim(bottom=0)
    plt.xlim(left=1)
    plt.xticks(range(0, int(max(df2.dil)), 3))
